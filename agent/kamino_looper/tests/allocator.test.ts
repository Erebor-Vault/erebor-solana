...
import { computeUsdcLoopApys } from "../src/strategy/apyScanner.js";
...
// Build a PortfolioState literal with derived totalValueUsdc.
function portfolio(p: {
  ...
  const idleUsdc = p.idleUsdc ?? 0;
  const ctokenBalance = p.ctokenBalance ?? 0;
  const suppliedUsdc = p.suppliedUsdc ?? 0;
  const borrowedUsdc = p.borrowedUsdc ?? 0;
  const healthFactor = p.healthFactor ?? (borrowedUsdc > 0 ? suppliedUsdc / borrowedUsdc : Infinity);
  return {
    idleUsdc,
    ctokenBalance,
    suppliedUsdc,
    borrowedUsdc,
    healthFactor,
    totalValueUsdc: idleUsdc + suppliedUsdc - borrowedUsdc,
  };
}

const goodLoops = computeUsdcLoopApys(0.06, 0.04, 1.5, 3.0);
const badLoops = computeUsdcLoopApys(0.02, 0.10, 1.5, 3.0); // empty
...
describe("decideAllocation", () => {
  ...
  it("emergency deleverages when HF < warning threshold", () => {
    const result = decideAllocation(
      portfolio({
        ctokenBalance: 50_000_000,
  ...
  it("does nothing when idle balance below minimum", () => {
    const result = decideAllocation(
      portfolio({ idleUsdc: 500_000 }), // 0.5 USDC
      goodLoops,
      baseConfig
    );
    expect(result.action).toBe("NONE");
