// tests/preset_kamino_liquidity.ts
// E2E test: apply the Kamino Liquidity preset to a fresh strategy on a local
...
import { getMint, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID } from "@solana/spl-token";
...
describe("Kamino Liquidity preset (e2e)", function () {
  ...
  before(async () => {
    // vaultId=80 — distinct from all other test suites
    fx = await setupVault({
      program,
      payer: admin,
      vaultId: 80,
  ...
  it("applies the preset and is detected as kamino_liquidity", async () => {
    const underlying = await getMint(provider.connection, fx.mint);

    const ctx: PresetBuildContext = {
      connection: provider.connection,
      program,
      cluster: "devnet", // picks PROTOCOL_REGISTRY.devnet → mock_kamino
      ...
      kaminoObligation: Keypair.generate().publicKey, // synthetic — value-source target only
    };

    const ixs = await KAMINO_LIQUIDITY.buildIxs(ctx);
    // 2 allowed_action + 2 auto_action_config + 1 value_source = 5
